...
# --input es la ruta del archivo que se quiere transformar
ap.add_argument("-i", "--input", required=True,
                help="ruta de la carpeta con las imágenes para transformar")

# --style es la ruta del archivo en el que se basa el estilo
ap.add_argument("-s", "--style", required=True,
                help="ruta de la carpeta de destino")

# --model es la ruta del modelo que se va a usar, en este caso VGG19
# ver: http://www.vlfeat.org/matconvnet/pretrained/
# nombre del archivo: imagenet-vgg-verydeep-19.mat
ap.add_argument("-m", "--model", required=True,
                help="ruta del modelo que se va a usar")

# --output es la ruta de la carpeta en donde se guardan las imagenes
...
# Inputs
file_content_image = args["input"]
file_style_image = args["style"]
...
n_checkpoints = 1            # number of checkpoints
n_iterations_checkpoint = 60   # learning iterations per checkpoint
...
def _conv2d_relu(prev_layer, n_layer, layer_name):
    ...
    conv2d = tf.nn.conv2d(prev_layer, filter=W, strides=[
        ...
        with tf.Session() as sess:
            ...
            if(total < 200):
                ...
                net['input'] = tf.Variable(
                    np.zeros((a, h, w, d), dtype=np.float32))
                ...
                def content_layer_loss(p, x):
                    # d: number of filters; h,w : height, width
                ...
                # Get activation output for content layer
